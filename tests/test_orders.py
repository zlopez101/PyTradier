...
def test_successful_orders(randomTicker, randomOption):
    ...
    equity = StopLimitOrder(randomTicker[0], "buy", 10, 10.5, 9)
    equity_details = equity.params("equity")
    ...
    assert not equity_details.get("duration"), "no duration"

    option = LimitOrder(randomOption[0], "sell_to_open", 1, 10.0, duration="gtc")
    option_details = option.params("option")
    ...
    assert option_details.get("duration") == "gtc"

    """
    make_legs() method. Takes in an index -> will be used in for loops of *args in complex order types
    """
    makeLegs = StopOrder(randomTicker[1], "buy", 10, 100.00, duration="gtc")
